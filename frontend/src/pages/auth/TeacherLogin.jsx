...
export default function TeacherLogin() {
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      ...
      if (data.email_sent === false && data.otp) {
        setOtp(data.otp)
        toast.error('⚠️ Email failed — OTP is shown below. Use it to sign in.', { duration: 10000 })
      } else {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left brand panel */}
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-900 flex-col items-center justify-center p-12 text-white">
        <div className="w-20 h-20 bg-white/10 rounded-3xl flex items-center justify-center mb-6 backdrop-blur">
          <AcademicCapIcon className="w-10 h-10 text-blue-300" />
        </div>
        <h1 className="text-4xl font-black mb-4">Teacher Portal</h1>
        <p className="text-blue-200 text-center text-lg max-w-sm">
          Create exams, manage your classes, send invitations, and track student results.
        </p>
        <div className="mt-12 space-y-3 w-full max-w-xs">
          {['Create & manage exams','Build question banks','Manage your classes','Send QR invitations','Track student results'].map(f => (
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-slate-50 dark:bg-slate-950">
        <div className="w-full max-w-md">
          {/* Back to home */}
          <div className="mb-6">
            <Link to="/" className="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors group">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Teacher Sign In</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Access your teaching dashboard</p>
          </div>
          ...
          {stage === 'credentials' && (
            <form onSubmit={handleRequestOtp} className="space-y-4">
              <div>
                ...
                <input type="email" className="input-field" placeholder="teacher@school.edu"
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-xl p-3">
                <p className="text-xs text-blue-700 dark:text-blue-300 flex items-center gap-2">
                  ...
                  A 6-digit OTP will be sent to your email for security
                </p>
              </div>
              ...
              <button type="submit" disabled={loading || !captchaValid}
                className="w-full py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-xl
                  hover:from-blue-600 hover:to-indigo-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {stage === 'otp' && (
            <form onSubmit={handleVerifyOtp} className="space-y-6">
              ...
              <OtpInput value={otp} onChange={setOtp} prefix="teacher-otp" />
              <button type="submit" disabled={loading || otp.length < 6}
                className="w-full py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-xl
                  hover:from-blue-600 hover:to-indigo-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
              ...
              <div className="text-center space-y-2">
                <button type="button" onClick={handleResend} disabled={resending}
                  className="text-sm text-blue-600 hover:underline">{resending ? 'Sending…' : 'Resend OTP'}</button>
                <br />
          ...
          {stage === 'forgot_reset' && (
            <form onSubmit={handleForgotReset} className="space-y-4">
              ...
              <OtpInput value={otp} onChange={setOtp} prefix="forgot-teacher-otp" />
          ...
          <div className="mt-6 text-center text-sm text-slate-500">
            ...
            <span className="text-slate-400">Contact your administrator to create one for you.</span>
            <br className="mb-2" />
            <Link to="/admin/login" className="text-slate-400 hover:underline text-xs mt-1 inline-block">Admin login</Link>
          </div>
        </div>
      </div>
    </div>
