...
import {
  AcademicCapIcon, EyeIcon, EyeSlashIcon, ShieldCheckIcon, EnvelopeIcon,
...
function OtpInput({ value, onChange }) {
  ...
  const handleKey = (i, e) => {
    ...
    if (!v && i > 0) document.getElementById(`reg-otp-${i - 1}`)?.focus()
  }
  const handlePaste = (e) => {
...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true)
    try {
      await authAPI.adminRegisterSendOtp({
      ...
      <div className="relative w-full max-w-md">
        <div className="glass rounded-3xl p-8 shadow-2xl">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Register as an exam administrator</p>
          </div>
            ...
            {['Details', 'Verify Email'].map((s, i) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold
                  transition-all ${i === 0 && stage === 'form' ? 'bg-primary-600 text-white'
                    : i === 0 ? 'bg-emerald-500 text-white'
                    : i === 1 && stage === 'otp' ? 'bg-primary-600 text-white'
          ...
          {stage === 'form' ? (
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                ...
                <input type="text" name="name" className="input-field" placeholder="Dr. John Smith"
                  value={form.name} onChange={handleChange} required />
              </div>
              <div>
                <label className="label">Email Address</label>
                <input type="email" name="email" className="input-field" placeholder="admin@school.edu"
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  ...
                  <span className="font-semibold text-primary-600">{form.email}</span>
                </p>
              </div>
